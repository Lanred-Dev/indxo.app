<script lang="ts">
    ...
    import type { SessionContext, SidebarVisiblityContext, SizesContext } from "./+layout.svelte";
    import { Popup, PopupContent, PopupTrigger } from "$lib/components/Popup";

    const { session, user }: SessionContext = getContext("session");
    const sizes: SizesContext = getContext("sizes");
    const sidebarVisibility: SidebarVisiblityContext = getContext("sidebarVisibility");
</script>
    ...
    >
        <div class="flex-center gap-3 pl-3">
            {#if session}
                <button
                    id="sidebarToggle"
                    onclick={() => sidebarVisibility().setVisible(!sidebarVisibility().isVisible)}
                >
                    <img
                        class="size-7"
        ...
        {#if session}
            <Popup>
                ...
                <PopupContent class="space-y-1 px-1! pb-1!">
